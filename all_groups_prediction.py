...
plt.plot(val_open.index, val_open["val_z_groups"])
plt.show()

val_open.corr()  # some correlation for wellknown_dylan x valence
sns.lmplot(data=val_open, y="val_z_groups", x="wellknown_dylan", x_jitter=.1)
plt.xticks([1., 2., 3., 4.], ["Not at all", "Somewhat", "Well", "Very Well"])
plt.title("Familiarity With Dylan vs. Valence")
plt.tight_layout()
plt.show()

sns.catplot(x="native_language_str", y="val_z_groups", kind="box", data=val_open.replace("English-German Bilingual", "Bilingual"))
...
val_open["val_z_abs"] = val_open["val_z_groups"].abs()
ols = sm.ols("val_z_groups ~ C(wellknown_dylan)  + C(native_language_str) + C(group)", data=val_open)
res = ols.fit()
...
y = val_open.loc[:, "val_z_groups"].to_numpy()
